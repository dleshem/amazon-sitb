...
describe('AmazonSitbClient', () => {
	...
	describe('bookData', () => {
		...
		it ('returns valid book data for valid asins', async () => {
			...
			const bookData = await sitbClient.bookData({
				asin: someAsin
			})
		...
		it ('gracefully fails on timeout', async () => {
			...
			try {
				const bookData = await sitbClientWithTimeout.bookData({
					asin: someAsin
				})

				// Unexpected success
				assert.ok(false, `bookData should have timed out, but returned ${JSON.stringify(bookData)}`)
			} catch (error) {
		...
		it ('gracefully fails when network is down', async () => {
			...
			try {
				const bookData = await sitbClientWithInvalidEndpoint.bookData({
					asin: someAsin
				})
	...
	describe('searchResults', () => {
		...
		it ('returns valid search results for a valid query', async () => {
			...
			const bookSearchResults = await sitbClient.searchResults({
				asin: someAsin,
				pageNumber: somePageNumber,
				pageSize: somePageSize,
				query: someQuery
		...
		it ('gracefully fails on timeout', async () => {
			...
			try {
				const bookSearchResults = await sitbClientWithTimeout.searchResults({
					asin: someAsin,
					pageNumber: somePageNumber,
					pageSize: somePageSize,
					query: someQuery
				...
				// Unexpected success
				assert.ok(false, `searchResults should have timed out, but returned ${JSON.stringify(bookSearchResults)}`)
			} catch (error) {
				expect(error.code).to.equal('timeout')
				expect(error.description).to.not.be.empty
			}
		...
		it ('gracefully fails when network is down', async () => {
			...
			try {
				const bookSearchResults = await sitbClientWithInvalidEndpoint.searchResults({
					asin: someAsin,
					pageNumber: somePageNumber,
					pageSize: somePageSize,
					query: someQuery
		...
		it ('gracefully fails on protocol error', async () => {
			...
			try {
				const bookSearchResult = await sitbClient.searchResults({
					asin: someAsin,
					pageNumber: somePageNumber,
					pageSize: somePageSize,
					query: someQuery
	...
	describe('goToPage', () => {
		...
		it ('returns valid book pages for a valid page with no token', async () => {
			driver.goToPage({
				asin: someAsin,
				page: somePage
			}).returns({
		...
		it ('returns empty book pages for an invalid page with no token', async () => {
			driver.goToPage({
				asin: someAsin,
